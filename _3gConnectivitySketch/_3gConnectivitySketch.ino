...
void loop(){
  ...
  char obd2char[30];
  char tempsensorschar[30];
  
   while(obd.available() > 0) // Don't read unless
                                                  // there you know there is data
   {
       if(index < 29) // One less than the size of the array
       {
           inChar = obd.read(); // Read a character
           obd2char[index] = inChar; // Store it
           index++; // Increment where to write next
           obd2char[index] = '\0'; // Null terminate the string
       }
   }
   
   while(temp.available() > 0) // Don't read unless
                                                  // there you know there is data
   {
       if(index < 29 i5) // One less than the size of the array
       {
           inChar = temp.read(); // Read a character
           tempsensorschar[index] = inChar; // Store it
           index++; // Increment where to write next
           tempsensorschar[index] = '\0'; // Null terminate the string
       }
   }
  
  
  String obd2(obd2char);
  String tempsensors(tempsensorschar);
